...
fn main() {
    ...
    match generate_report(&project_root) {
        Ok(()) => {
            println!(
                "cargo:warning=✅ Requirements report generated at target/requirements_report.html"
            );
        }
        Err(e) => {
...
fn generate_report(project_root: &Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if trace_result.is_success {
        ...
        println!(
            "cargo:warning=⚠️ {} defects found - see report for details",
