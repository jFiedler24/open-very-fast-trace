...
fn run_ovft(matches: &ArgMatches) -> Result<()> {
    ...
    let project_root = find_cargo_project_root(&current_dir)
        .context("Not in a Cargo project directory")?;
    ...
    let trace_result = tracer.trace().context("Failed to run requirements traceability analysis")?;
    ...
    if check_mode {
        if trace_result.defect_count > 0 {
            eprintln!("❌ Found {} defects in requirements traceability", trace_result.defect_count);
            std::process::exit(1);
        } else {
...
fn find_cargo_project_root(start_dir: &PathBuf) -> Option<PathBuf> {
